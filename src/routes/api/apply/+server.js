...
export async function POST({ request }) {
  try {
    ...
    // 验证必填字段
    const requiredFields = ['companyName', 'creditCode', 'legalPerson', 'legalPhone', 'city', 'loanAmount', 'loanPurpose', 'loanTerm'];
...
export async function GET({ url }) {
  try {
    ...
    const collection = await getApplicationsCollection();
    const application = await collection.findOne({ '申请编号': applicationId });
